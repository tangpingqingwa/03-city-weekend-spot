...
test("maker footer keeps poster styling, visible focus, and mobile fit", () => {
  assert.match(cssSource, /\.city-weekend-sheet \.maker-footer\s*\{/);
  ...
  assert.match(
    cssSource,
    /@media \(max-width: 40rem\)[\s\S]*?\.city-weekend-sheet \.maker-footer\s*\{[\s\S]*?font-size:\s*0\.72rem/,
  );
});
