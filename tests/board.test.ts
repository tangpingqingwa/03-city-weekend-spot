...
test("occupied later ranks stay quieter than occupied #1 venue", () => {
  ...
  assert.ok(hop > quiet && book > hop && bid > book);
  assert.match(laterCard, /data-rank="2"/);
  assert.match(laterCard, /data-later-book=""/);
...
test("occupied NYC #1 $bid stays a later fact and does not shout beside the venue", () => {
  const css = readFileSync(join(process.cwd(), "src", "app", "board.css"), "utf8");
  const prizeSize = css.match(
    /clamp\(([\d.]+)rem, 9vw, 4\.4rem\)/,
  );
  const laterFact = css.match(
    /\.number-one\[data-prize-before-price\] \.bid\.later-fact\[data-later-fact\]\s*\{([^}]*)\}/,
  );
  const clickFact = css.match(
    /\.number-one\[data-prize-before-price\] \.clicks\.later-fact\s*\{([^}]*)\}/,
  );
  assert.ok(prizeSize);
  assert.ok(laterFact);
  assert.ok(clickFact);
  const bidSize = laterFact[1].match(/font-size:\s*([\d.]+)rem/);
  const clickSize = clickFact[1].match(/font-size:\s*([\d.]+)rem/);
  assert.ok(bidSize);
  assert.ok(clickSize);
  assert.ok(Number(prizeSize[1]) > Number(bidSize[1]));
  assert.ok(Number(prizeSize[1]) > Number(clickSize[1]));
  assert.match(laterFact[1], /color:\s*var\(--muted\)/);
  assert.match(laterFact[1], /font-weight:\s*500/);
  assert.doesNotMatch(laterFact[1], /color:\s*var\(--accent\)/);
  assert.match(clickFact[1], /color:\s*var\(--muted\)/);
  ...
  const bidClass = onlyCard.indexOf('class="bid later-fact"');
  const bid = onlyCard.indexOf('data-bid=""');
  const clicks = onlyCard.indexOf("data-clicks");
  assert.ok(prize >= 0 && venue > prize);
  assert.ok(book > venue && laterFactStamp > book && bidClass >= 0);
  assert.ok(bid > book && laterFactStamp >= bidClass);
  assert.ok(clicks > laterFactStamp);
  assert.match(onlyCard, /data-prize-before-price=""/);
  assert.match(onlyCard, /class="weekend-answer"/);
  assert.match(onlyCard, /class="bid later-fact"/);
  assert.match(onlyCard, /data-later-fact=""/);
  assert.match(onlyCard, /class="clicks later-fact"/);
  assert.match(onlyCard, /\$12/);
  assert.match(onlyCard, /4 clicks/);
  assert.match(onlyCard, /class="book-one"/);
  ...
  assert.doesNotMatch(laterCard, /later-fact/);
  assert.doesNotMatch(laterCard, /data-prize-before-price|data-prize=/);
  assert.match(laterCard, /data-later-book=""/);
  assert.match(laterCard, /data-later-quiet=""/);
  assert.match(laterCard, /data-book-later=""/);
  ...
  const occupiedBidClass = html.indexOf('class="bid later-fact"');
  ...
  assert.ok(occupiedBook > occupiedName && occupiedLaterFact > occupiedBook);
  assert.ok(occupiedBidClass >= 0 && occupiedBid > occupiedBook);
  assert.ok(occupiedClicks > occupiedLaterFact && laterHop > occupiedClicks);
  assert.ok(form > laterHop);
  assert.equal((html.match(/data-later-fact=""/g) ?? []).length, 1);
  assert.equal((html.match(/class="bid later-fact"/g) ?? []).length, 1);
  assert.equal((html.match(/data-prize=""/g) ?? []).length, 1);
  assert.equal((html.match(/data-book-number-one/g) ?? []).length, 1);
  assert.equal((html.match(/data-later-quiet=""/g) ?? []).length, 2);
  assert.match(html, /data-occupied="true"/);
  ...
  assert.match(html, /Claim #1 for/);
  assert.match(html, /action="\/api\/checkout"/);
  assert.doesNotMatch(html.slice(0, occupiedPrize), /data-later-fact|later-fact/);
  assert.doesNotMatch(html.slice(laterHop), /data-later-fact|class="bid later-fact"|data-prize=/);
  assert.doesNotMatch(html, /data-empty-board/);
  assert.doesNotMatch(html, /data-list-after-book-nine|data-book-after-list-eight/);
  assert.doesNotMatch(html, /map|leaflet|google\.maps|OpenStreetMap/i);
  assert.doesNotMatch(html, /★|4\.8|star-rating|data-stars|review count|rated 4\.9/i);
...
test("empty NYC weekend stays unpublished without occupied chrome", () => {
  ...
  assert.doesNotMatch(empty, /data-prize-before-price|data-prize=/);
  assert.doesNotMatch(empty, /data-later-fact|later-fact/);
